...
function injectPill() {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "HIGHLIGHT_ELEMENT" && message.selector) {
      ...
      updatePillUI(pill, isPillActive);
    } else if (message.type === "UPDATE_VOICE_ACTIVITY") {
      setScreenHaloVoiceLevel(message.level);
      ...
      processImageWithGrid(message.dataUrl)
      ...
      executeCursorCommands(message.commands)
        .then(() => {
...
// ============================================================================
// SCREEN HALO - Subtle violet frame around the viewport while recording
// ============================================================================
...
function injectScreenHalo() {
  ...
  style.textContent = `
        ...
        .jumeau-halo-frame {
            ...
            border: 12px solid rgba(168, 85, 247, 0.58);
            opacity: 0.36;
            box-shadow:
  ...
  wrapper.innerHTML = '<div class="jumeau-halo-frame"></div>';
...
function setScreenHaloActive(isActive) {
  ...
  if (isActive) {
    ...
    setScreenHaloVoiceLevel(screenHaloVoiceLevel);
  } else {
    screenHaloWrapper.classList.remove("active");
    setScreenHaloVoiceLevel(0);
  }
}

function setScreenHaloVoiceLevel(level) {
  screenHaloVoiceLevel = Math.max(0, Math.min(1, Number(level) || 0));

  if (!screenHaloWrapper) injectScreenHalo();
  if (!screenHaloWrapper) return;

  const frame = screenHaloWrapper.querySelector(".jumeau-halo-frame");
  if (!frame) return;

  const borderAlpha = 0.58 + screenHaloVoiceLevel * 0.32;
  const strokeAlpha = 0.42 + screenHaloVoiceLevel * 0.34;
  const nearGlowAlpha = 0.12 + screenHaloVoiceLevel * 0.26;
  const farGlowAlpha = 0.08 + screenHaloVoiceLevel * 0.18;
  const nearGlowSize = 10 + screenHaloVoiceLevel * 18;
  const farGlowSize = 24 + screenHaloVoiceLevel * 38;
  const opacity = 0.36 + screenHaloVoiceLevel * 0.58;

  frame.style.opacity = opacity.toFixed(3);
  frame.style.borderColor = `rgba(168, 85, 247, ${borderAlpha.toFixed(3)})`;
  frame.style.boxShadow = `
        0 0 0 1px rgba(168, 85, 247, ${strokeAlpha.toFixed(3)}),
        0 0 ${nearGlowSize.toFixed(1)}px rgba(168, 85, 247, ${nearGlowAlpha.toFixed(3)}),
        0 0 ${farGlowSize.toFixed(1)}px rgba(139, 92, 246, ${farGlowAlpha.toFixed(3)})
    `;

  if (!screenHaloWrapper.classList.contains("active")) {
